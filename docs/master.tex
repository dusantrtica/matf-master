...
kvalitetniji raspored) i izgradnja veb interfejsa za praktičnu upotrebu.
...
dodele predavanja, vežbi i laboratorijskih časova u raspoložive
...
\item \textbf{Meka ograničenja} (eng. \emph{soft constraints}) --
  ...
  \item Minimizacija broja promena učionica za jednu grupu u toku dana.
...
rešavača. Ovaj pristup se zasniva na \emph{lenjem generisanju klauza}
...
\item \textbf{Settings} -- podesavanja rasporeda:
  ...
  \item \texttt{startHour}, \texttt{endHour} -- početak i kraj
    radnog vremena (8--20),
  \item \texttt{duration} -- trajanje jednog časa (1 sat).
  \end{itemize}
...
\item \textbf{Generisanje sesija.}
  ...
  to nalaze kvote: \texttt{quota.theory} sesija tipa ``predavanje''
...
Ovo jedino globalno ograničenje obuhvata sve sesije i garantuje
...
\textbf{HC4: Ravnomerna raspodela po danima.}
Za svaku grupu $g$ i svaki dan $d$, broj sesija te grupe u tom
danu je ograničen odozgo:

\begin{equation}
\sum_{s \in \mathcal{S}_g} \sum_{h,r} x_{s,d,h,r} \leq
  \left\lceil \frac{N_g}{D} \right\rceil,
  \quad \forall g, \forall d
\label{eq:mip_hc4}
\end{equation}

gde je $N_g = |\mathcal{S}_g|$ ukupan broj sesija grupe $g$.
Ovo ograničenje forsira ravnomerniju raspodelu časova tokom nedelje.

\textbf{Napomena:} Ograničenje HC4 postoji samo u MIP formulaciji.
CP formulacija nema ekvivalentno ograničenje, što znači da dopustivi
skupovi nisu potpuno identični, ali su oba konzistentna sa realnim
zahtevima raspoređivanja.
...
\midrule
Sesije                 & \multicolumn{2}{c}{(popuniti iz benchmark rezultata)} \\
Promenljive            & --      & -- \\
Ograničenja            & --      & -- \\
Vreme konstrukcije     & --      & -- \\
Vreme rešavanja        & --      & -- \\
...
\textbf{Napomena:} Vrednosti u tabelama treba popuniti konkretnim
rezultatima iz pokretanja benchmark-a. Rezultate je moguće dobiti
komandom:
...
Za MATF-L skalu sa $D=5$ dana, $H=12$ sati i $R=35$ učionica,
svaka sesija generiše do $5 \cdot 12 \cdot 35 = 2100$ binarnih
...
zgrada N, Jagićeva), pa bi promena zgrade između dva uzastopna časa
