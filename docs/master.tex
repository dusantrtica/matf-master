...
\texttt{rule\_join\_same\_classes.py}) grupiše časove istog predmeta,
...
\texttt{rule\_single\_location\_in\_day\_for\_group.py}) sprečava da
...
\texttt{rule\_no\_gaps\_in\_schedule.py}) cilja na kvalitet rasporeda
...
\texttt{test\_no\_gaps\_in\_schedule.py}, koji potvrđuje da rešenje na
...
nadogradnju sistema za raspoređivanje časova. U glavi
\ref{chp:nadogradnja} prikazana je ta nadogradnja: uvedena je
arhitektura pravila (eng. \emph{plugin}) sa podrškom za tvrda i meka
ograničenja, funkcija cilja koja minimizuje težinsku sumu prekršaja, i
tri nova ograničenja -- spajanje časova u blokove, jedna lokacija po
danu za grupu i izbegavanje procepa u rasporedu. Time je pokazano da se
CP-SAT model lako i modularno proširuje ka realnijim scenarijima.

\section{Dalji rad}

U glavi \ref{chp:nadogradnja} već su implementirana meka ograničenja sa
funkcijom cilja, lokacijska ograničenja za grupe i grupisanje časova u
blokove. Preostali pravci daljeg rada su:

\begin{enumerate}
\item \textbf{Raspoređivanje nastavnog osoblja.}
  Uključivanje profesora i asistenata u model, sa ograničenjima
  njihove raspoloživosti i preferencija, kao i lokacijskim
  ograničenjima za nastavnike (početak modula \texttt{src/rules/staff}).

\item \textbf{Proširena ograničenja učionica.}
  Uzimanje u obzir kapaciteta učionice u odnosu na veličinu grupe.

\item \textbf{Dodatna meka ograničenja kvaliteta.}
  Na primer, ravnomerna raspodela časova kroz nedelju i izbegavanje
  kasnih termina, uz fino podešavanje težina u funkciji cilja.
